...
No other libraries need to be installed to use the `mecRecordLinkage` package. All functionalities are build in. For preparing data sets, using `reclin2` packages is proposed inside of the `mecRecordLinkage`, what is the only dependency in the library.
...
```{r}
model_ER2 <- mec(A = data1, B = data2, vars = vars, blockvars = "dobyear", control = control_mec(theta_est = "2", increase_rate = .6), error_rate = TRUE)
model_ER2$M
```
